 ...
 * 
 * <p>Fornece endpoints para operações CRUD da tabela FORNECEDOR do sistema fasiclin.
 * Este controller acessa os dados reais do banco de dados MySQL.</p>
 * 
...
public class FornecedorController {
    ...
    @Operation(summary = "Listar todos os fornecedores", 
               description = "Retorna uma lista completa de todos os fornecedores cadastrados no sistema, ordenados por representante")
    ...
    @Operation(summary = "Buscar fornecedor por ID", 
               description = "Retorna um fornecedor específico baseado no ID fornecido")
    @ApiResponses(value = {
    ...
    public ResponseEntity<Fornecedor> findById(
            @Parameter(description = "ID do fornecedor a ser buscado", required = true, example = "1")
            @PathVariable 
            @NotNull 
     ...
     */
    @Operation(summary = "Buscar fornecedor por ID de pessoa", 
               description = "Retorna o fornecedor associado ao ID de pessoa fornecido")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Fornecedor encontrado com sucesso"),
    ...
    public ResponseEntity<Fornecedor> findByIdPessoa(
            ...
            @PathVariable 
            @NotNull 
            @Min(value = 1, message = "ID da pessoa deve ser maior que 0") 
            Integer idPessoa) {
     ...
     */
    @Operation(summary = "Buscar fornecedores por representante", 
               description = "Retorna fornecedores que contenham o termo especificado no campo representante (busca case-insensitive)")
    @ApiResponse(responseCode = "200", description = "Lista de fornecedores retornada com sucesso")
    @GetMapping("/buscar/representante")
    public ResponseEntity<List<Fornecedor>> findByRepresentante(
            @Parameter(description = "Termo de busca para representante", required = true, example = "João")
            @RequestParam 
            @NotNull 
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "400", description = "Dados inválidos fornecidos",
                     content = @Content),
        @ApiResponse(responseCode = "409", description = "Conflito - já existe fornecedor para esta pessoa",
                     content = @Content)
    })
    @PostMapping
     ...
     */
    @Operation(summary = "Buscar fornecedores por produto", 
               description = "Retorna todos os fornecedores que fornecem o produto especificado")
    ...
    public ResponseEntity<List<Fornecedor>> findByProdutoId(
            ...
            @PathVariable @NotNull @Min(1) Integer produtoId) {
        // Por enquanto, retorna todos os fornecedores - pode ser implementada lógica específica depois
        List<Fornecedor> fornecedores = fornecedorService.findAllOrderByRepresentante();
        return ResponseEntity.ok(fornecedores);
    }
}