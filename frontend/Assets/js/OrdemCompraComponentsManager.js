...
class OrdemCompraComponentsManager {
  ...
  async openViewModal(ordem) {
    ...
    // Guardar valor da ordem para comparação com a soma dos itens
    this.currentOrdemValor = Number(ordem.valor) || 0;
    // Guardar valor da ordem para comparação com soma dos itens
    this.currentOrdemValor = Number(ordem?.valor) || 0;
    if (viewDataOrdem)
    ...
    if (viewDataEntre)
      viewDataEntre.textContent = ordem.dataEntre
        ? this.formatDate(ordem.dataEntre)
        : "Não informada";
  ...
  renderOrdemItens(itens, attempt = 0) {
                                            ...
                                            <div class="financial-summary-view">
                                                ...
                                                <div class="summary-content">
                                                    <div class="summary-row">
                                                        ...
                                                        <span class="summary-value" id="viewValorOrdem">R$ 0,00</span>
                                                    </div>
                                                    <div class="summary-row">
                                                        <span class="summary-label">
                                                            <i data-feather="hash"></i>
    ...
    if (diferencaElement) {
      const diff = (this.currentOrdemValor || 0) - valorTotal;
      diferencaElement.textContent = this.formatCurrency(diff);
      diferencaElement.style.color =
        Math.abs(diff) < 0.005 ? "#16a34a" : "#eab308";
    }
  ...
  atualizarResumoFinanceiro() {
    ...
    if (this.elements.subtotalOrdem)
      this.elements.subtotalOrdem.textContent = this.formatCurrency(subtotal);
    if (this.elements.totalOrdem)
      this.elements.totalOrdem.textContent = this.formatCurrency(subtotal);
  }
  ...
  limparFormularioItem() {
    ...
    if (this.elements.precoUnitarioInput)
      this.elements.precoUnitarioInput.value = "";
