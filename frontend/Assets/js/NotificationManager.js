...
class NotificationManager {
    ...
    show(message, options = {}) {
        ...
        if (!message || typeof message !== 'string') {
            console.error('[NotificationManager] Mensagem é obrigatória e deve ser uma string');
            return null;
        }
    ...
    createElement(id, message, config) {
        ...
        const contentHtml = config.title 
            ? `<div class="notification-title">${config.allowHtml ? config.title : this.escapeHtml(config.title)}</div>
               <div class="notification-message">${config.allowHtml ? message : this.escapeHtml(message)}</div>`
            : `<div class="notification-message">${config.allowHtml ? message : this.escapeHtml(message)}</div>`;
